...
export class ListComponent implements OnInit, OnDestroy {
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msRawardDivision/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msRawardDivision/manage/C/NEW`]);
    ...
    async onSearchComplete(list: any) {
        ...
        await list.map((item) => {       
            if (item.EFFECTIVE_DATE) {
                item.EFFECTIVE_DATE = toLocalShort(item.EFFECTIVE_DATE);
            }
        })
