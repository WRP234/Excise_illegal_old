...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-99-11');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msRelation/manage/R/${masterID}`]);
    }


    clickNew = () => this._router.navigate([`/msRelation/manage/C/NEW`]);
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasRelationshipgetByCon", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
