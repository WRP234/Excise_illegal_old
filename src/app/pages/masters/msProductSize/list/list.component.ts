...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-20-06');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msProductSize/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msProductSize/manage/C/NEW`]);
    ...
    clickSearch(Textsearch: any) {
        ...
        this.MasService.getByKeyword("MasProductSizegetByKeyword", paramsOther).then(list => {
            if(list.SUCCESS){
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasProductSizegetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
