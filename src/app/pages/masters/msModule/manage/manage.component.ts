    ...
    ShowMasterData() {
        ...
        this.MasService.getByKeyword("MasCourtgetByCon", paramsOther).then(list => {
            if (list.RESPONSE_DATA.length > 0) {
                ...
                this.ShowAlertError("พบปัญหาที่ API MasCourtgetByCon");
                this.preloader.setShowPreloader(false);
                this.router.navigate(['/msCourt/list']);
            }
        }, (err: HttpErrorResponse) => {
            this.ShowAlertError("API MasCourtgetByCon :: " + err.message);
        });
    }

    async OnSave() {
        if (this.COURT_CODE == null || this.COURT_CODE == undefined
            || this.COURT_NAME == null || this.COURT_NAME == undefined) {
