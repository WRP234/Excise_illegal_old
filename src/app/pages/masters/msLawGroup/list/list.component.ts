...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        ...
        this.sidebarService.setVersion('Masters 0.0.0.3');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msLawGroup/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msLawGroup/manage/C/NEW`]);
    ...
    clickSearch(Textsearch: any) {
        ...
        this.MasService.getAPI1111("MasLawGroupgetByKeyword", paramsOther).then(list => {
            if(list){
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getAPI1111("MasLawGroupgetByConAdv", form.value).then(list => {
            if(list){
                this.onSearchComplete(list);
                this.preloader.setShowPreloader(false);
            } else {
