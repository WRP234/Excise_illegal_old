...
export class ManageComponent extends MastersConfig {
    ...
    async ngOnInit() {
        ...
        if (this.mode == "R") {
            ...
            this.NATIONALITY_ID = "Auto Generate";
            this.chk1 = true;
    ...
    ShowMasterData(){
        ...
        this.MasService.getByKeyword("MasNationalitygetByCon", paramsOther).then(list => {
            if(list.RESPONSE_DATA.length > 0){
                this.NATIONALITY_NAME_TH = list.RESPONSE_DATA[0].NATIONALITY_NAME_TH;
                this.NATIONALITY_NAME_EN = list.RESPONSE_DATA[0].NATIONALITY_NAME_EN;
                this.IS_ACTIVE = list.RESPONSE_DATA[0].IS_ACTIVE;
                this.chk1 = `${this.IS_ACTIVE == "1" ? true : false}`;
                
                this.preloader.setShowPreloader(false);
            } else {
                this.ShowAlertError("พบปัญหาที่ API MasNationalitygetByCon");
                this.preloader.setShowPreloader(false);
                this.router.navigate(['/msNationality/list']);
            }
        }, (err: HttpErrorResponse) => {
            this.ShowAlertError("API MasNationalitygetByCon :: " + err.message);
        });
    }

    async OnSave() {
        if (this.NATIONALITY_NAME_TH == null || this.NATIONALITY_NAME_TH == undefined
            || this.NATIONALITY_NAME_EN == null || this.NATIONALITY_NAME_EN == undefined) {
    ...
    setData(){
        this.IS_ACTIVE = `${this.chk1 ? "1" : "0"}`;
        this.NATIONALITY_ID = `${this.NATIONALITY_ID == "Auto Generate" ? "" : this.NATIONALITY_ID}`;

        this.oParam = {
            NATIONALITY_ID: this.NATIONALITY_ID,
            NATIONALITY_NAME_TH: this.NATIONALITY_NAME_TH,
            NATIONALITY_NAME_EN: this.NATIONALITY_NAME_EN,
    ...
    async onIns() {
        ...
        this.MasService.MasinsAll("MasNationalityinsAll", this.oParam).then(async item => {
            if (item.SUCCESS) {
                this.NATIONALITY_ID = item.RESPONSE_DATA;
                this.ShowAlertSuccess(Message.saveComplete);
                this.onComplete();

                this.preloader.setShowPreloader(false);
                this.router.navigate([`/msNationality/manage/R/${this.NATIONALITY_ID}`]);
            } else {
    ...
    async onUdp() {
        ...
        this.MasService.MasUpdAll("MasNationalityupdAll", this.oParam).then(async item => {
            if (item.SUCCESS) {
                this.ShowAlertSuccess(Message.saveComplete);
