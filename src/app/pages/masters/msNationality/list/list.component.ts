...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-99-12');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msNationality/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msNationality/manage/C/NEW`]);
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasNationalitygetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
