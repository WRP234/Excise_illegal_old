...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-99-08');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msProvince/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msProvince/manage/C/NEW`]);
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasProvincegetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
