...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-99-10');
        this.sidebarService.setVersion('Masters 0.0.0.1');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msSubDistrict/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msSubDistrict/manage/C/NEW`]);
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasSubDistrictgetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
