...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60_M_16_00_01_00');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msWarehouse/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msWarehouse/manage/C/NEW`]);
    ...
    clickSearch(Textsearch: any) {
        ...
        this.MasService.getByKeyword("MasWarehousegetByKeyword", paramsOther).then(list => {
            if(list.SUCCESS){
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasWarehousegetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
    ...
    async onSearchComplete(list: any) {
        ...
        await list.map((item) => {
            ...
            switch (item.WAREHOUSE_TYPE) {
                case 0:
                    item.WAREHOUSE_TYPE_NAME = "คลังสินค้าของหน่วยงานภายใน";
                    break;
                case 1:
                    item.WAREHOUSE_TYPE_NAME = "คลังสินค้าของหน่วยงานภายนอก";
                    break;
            }
        })
