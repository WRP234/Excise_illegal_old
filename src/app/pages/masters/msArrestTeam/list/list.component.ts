...
export class ListComponent implements OnInit {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60_M_04_00_01_00');
    ...
    clickView(masterID: string) {
        this._router.navigate([`/msArrestTeam/manage/R/${masterID}`]);
    }

    clickNew = () => this._router.navigate([`/msArrestTeam/manage/C/NEW`]);
    ...
    clickSearch(Textsearch: any) {
        ...
        this.MasService.getByKeyword("MasArrestTeamgetByKeyword", paramsOther).then(list => {
            if(list.SUCCESS){
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasArrestTeamgetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
