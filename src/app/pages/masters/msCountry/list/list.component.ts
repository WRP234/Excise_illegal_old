...
export class ListComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        localStorage.setItem('programcode', 'ILG60-99-07');
    ...
    clickNew = () => this._router.navigate([`/msCountry/manage/C/NEW`]);
    ...
    onAdvSearch(form: any) {
        ...
        this.MasService.getByConAdv("MasCountrygetByConAdv", form.value).then(list => {
            if(list.SUCCESS){
                this.onSearchComplete(list.RESPONSE_DATA);
                this.preloader.setShowPreloader(false);
            } else {
