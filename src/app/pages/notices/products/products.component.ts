...
import { NoticeProductFormControl, NoticeProduct, ProductGroup } from '../notice-product'
...
export class ProductsComponent implements OnInit {
  ...
  advSearch = new BehaviorSubject<Boolean>(true);
  ...
  constructor(private noticeService: NoticeService,
    ...
    private _manageComponent: ManageComponent,
    ...
    this.ProductsFG = this.fb.group({
      ...
      PRODUCT_SELECT: this.fb.array([]) //this.createProductForm()
    })
  }
  ...
  async ngOnInit() {
    ...
    // await this.noticeService.MasProductGROUPCategoryForLiquorgetByCon().then(res => {
    //   // this.ProductGroup = res;
    //   console.log('res : ', res)
    //   let res1: any[] = [];
    //   let result: any[] = [];
    ...
    this._NoticeProduct = this._manageComponent.NoticeProduct
    this._noticeForm = this._manageComponent.noticeForm
  }
  ...
  public async checkOne(e: any, value: any, i: any) {
    ...
    if (checkbox.checked) {
      const size = this._product_dtl.at(i).get('SIZES').value;
      this._product_dtl.at(i).patchValue({ IsChecked: true, SIZES_TEMP: size });
      ...
      const index = form.findIndex(x => x['PRODUCT_MAPPING_ID'] == value['PRODUCT_MAPPING_ID']);
  ...
  async selectOptionChange(name: string, ev: any, ) {
    ...
    //   await this.noticeService.MasProductCategoryGroupgetByCon('', product_code).then(res => {
    ...
    //   await this.noticeService.MasProductCategoryRDBgetByCon(_category_group, _product_code).then(res => {
  ...
  async searchCreateProduct(value: any) {
    ...
    await this.preLoaderService.setShowPreloader(true);
    await this.noticeService.MasProductMappinggetByConAdv(value).then(res => {
      ...
      this.PRODUCT_FROMSEARCH.map(m => {
        ...
        m.REMARK = "";//Set for Product Selects
        if (m.IS_DOMESTIC == 4) { m.IS_DOMESTIC_NAME = "ไม่ระบุ" }
        else if (m.IS_DOMESTIC == 1) { m.IS_DOMESTIC_NAME = "ในประเทศ" }
        else if (m.IS_DOMESTIC == 2) { m.IS_DOMESTIC_NAME = "สถานบริการ" }
        else if (m.IS_DOMESTIC == 3) { m.IS_DOMESTIC_NAME = "ต่างประเทศ" }
      ...
      this.onSearchComplete(product_dtl);
    })
  }

  public onSearchByKeyWord(form: any) {
    this.preLoaderService.setShowPreloader(true);
    this.noticeService.MasProductMappinggetByKeyword(form).then(res => {
      const product_dtl = res;
      product_dtl.map(m => m.IsChecked = false);
      this.PRODUCT_FROMSEARCH = res;
      this.PRODUCT_FROMSEARCH.map(m => {
  ...
  async calQuanitity(ev: number, index: any) {
    const PRODUCT_GROUP_ID = this.ProductsFG.controls['PRODUCT_SELECT'].value[index].PRODUCT_GROUP_ID;
    const SIZES_UNIT = this.ProductsFG.controls['PRODUCT_SELECT'].value[index].SIZES_UNIT;
    const TEMP = this.ProductsFG.controls['PRODUCT_SELECT'].value[index];
    let _SIZE: number = (ev * Number(TEMP.SIZES));
    let toFixed: number = 0;
    switch (PRODUCT_GROUP_ID) {
      case 1: toFixed = 3; break;
      case 2: toFixed = 3; break;
      case 6: _SIZE = Number(TEMP.SIZES); break;
      case 7: _SIZE = Number(TEMP.SIZES); break;
      case 8: _SIZE = Number(TEMP.SIZES); break;
      case 9: toFixed = 3; break;
      case 13: toFixed = 3; break;
      case 14: SIZES_UNIT == 'มวน' ? toFixed = 0 : toFixed = 3; break;
      default: break;
    }
    this._product_select.at(index).get('VOLUMN').patchValue(_SIZE.toFixed(toFixed));
  }
  ...
  private createProductForm(): FormGroup {
    Product_dtlFormControl.CO2 = new FormControl(null),
      ...
      //CTs
      Product_dtlFormControl.QUANTITY_UNIT = new FormControl(null),
      Product_dtlFormControl.QUANTITY = new FormControl(null),
      Product_dtlFormControl.PRODUCT_DESC = new FormControl(null),
      Product_dtlFormControl.REMARK = new FormControl(null),
      Product_dtlFormControl.SIZES_TEMP = new FormControl(null)


    return this.fb.group(Product_dtlFormControl)
  }
  ...
  addProduct() {
    ...
    product_select.map(m => {
      m.PRODUCT_ID = '',
        ...
        m.PRODUCT_DESC = m.PRODUCT_NAME_DESC,
        m.SUGAR = m.SUGAR,
        m.CO2 = m.CO2,
        m.DEGREE = m.DEGREE,
        m.PRICE = m.PRICE,
        ...
        m.IsNewItem = true
      this._NoticeProduct.push(this.fb.group(m));
      console.log('m : ', m)
    })
    // let addProd = {
    //   PRODUCT_ID: '',
    ...
    console.log('addProduct this._noticeForm : ', this._noticeForm)
